...
def log_validation(
    ...
    # We dataset on the simplified learning objective. If we were previously predicting a variance, we need the scheduler to ignore it
    scheduler_args = {}
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=512,
        help=(
            "The resolution for input images, all the images in the dataset/validation dataset will be resized to this"
            " resolution"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_text_encoder",
        action="store_true",
        help="Whether to dataset the text encoder. If set, the text encoder should be float32 precision.",
    )
    parser.add_argument(
...
def main(args):
    ...
    logger.info(f"  Total dataset batch size (w. parallel, distributed & accumulation) = {total_batch_size}")
